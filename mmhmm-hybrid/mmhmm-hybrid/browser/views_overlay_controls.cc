...
void ViewsOverlayControls::Initialize(CefRefPtr<CefWindow> window,
  ...
  panel_controller_ =
      window->AddOverlayView(panel_, CEF_DOCKING_MODE_TOP_RIGHT, true);
  panel_controller_->SetVisible(true);

  // Menu button.
  menu_button->SetBackgroundColor(kBackgroundColor);
  menu_controller_ =
      window_->AddOverlayView(menu_button, CEF_DOCKING_MODE_TOP_LEFT, true);
  ...
  location_controller_ =
      window_->AddOverlayView(location_bar_, CEF_DOCKING_MODE_CUSTOM, true);
}
