...
CefRefPtr<ViewsWindow> ViewsWindow::Create(
    ...
    base::RepeatingCallback<bool()> before_close_callback) {
  ...
  CefRefPtr<CefBrowserView> browser_view = CefBrowserView::CreateBrowserView(
      client, url, settings, extra_info, request_context, views_window);
  ...
  views_window->before_close_callback_ = std::move(before_close_callback);
...
void ViewsWindow::SetSize(size_t width, size_t height) {
  CEF_REQUIRE_UI_THREAD();
  if (window_)
    window_->SetSize({static_cast<int>(width), static_cast<int>(height)});
}

void ViewsWindow::SetMinimumSize(size_t p_width, size_t p_height) {
  CEF_REQUIRE_UI_THREAD();
  int width = static_cast<int>(p_width);
  int height = static_cast<int>(p_height);
  minimum_window_size_ = {width, height};
#if defined(OS_MAC)
  CefWindowHandle windowHandle = window_->GetWindowHandle();
  Airtime::SwiftBridgeSize size = Airtime::SwiftBridgeSize::init(static_cast<double>(p_width), static_cast<double>(p_height));
  Airtime::SwiftBridge::setMinSizeOnWindow(windowHandle, size);
#else
  auto current_size = window_->GetSize();
  // Resize the window if it is below the minimum size
  if (current_size.width < width || current_size.height < height) {
    window_->SetSize({std::max(current_size.width, width),
                      std::max(current_size.height, height)});
  }
#endif
}

void ViewsWindow::SetMaximumSize(size_t p_width, size_t p_height) {
  CEF_REQUIRE_UI_THREAD();
  int width = static_cast<int>(p_width);
  int height = static_cast<int>(p_height);
  maximum_window_size_ = {width, height};
#if defined(OS_MAC)
  CefWindowHandle windowHandle = window_->GetWindowHandle();
  Airtime::SwiftBridgeSize size = Airtime::SwiftBridgeSize::init(static_cast<double>(p_width), static_cast<double>(p_height));
  Airtime::SwiftBridge::setMaxSizeOnWindow(windowHandle, size);
#else
  auto current_size = window_->GetSize();
  // Resize the window if it is above the maximum size
  if (current_size.width > width || current_size.height > height) {
    window_->SetSize({std::min(current_size.width, width),
                      std::min(current_size.height, height)});
  }
#endif
}
...
void ViewsWindow::SetTitle(const std::string& title) {
  ...
  // mmhmm Hybrid #325 - Don't allow updating of window title from web
#else
  if (window_)
...
void ViewsWindow::OnWindowCreated(CefRefPtr<CefWindow> window) {
  ...
  if (with_controls_) {
    ...
    maximum_window_size_ = CefSize(INT_MAX, INT_MAX);
  }
...
bool ViewsWindow::CanClose(CefRefPtr<CefWindow> window) {
  ...
  #if defined(OS_MAC)
  if (Airtime::SwiftBridge::canCloseWindow(window->GetWindowHandle()) == false) {
    return false;
  }
  #endif

  if (!before_close_callback_.is_null()) {
    if (!before_close_callback_.Run())
      return false;
  }
  ...
  CefRefPtr<CefBrowser> browser = browser_view_->GetBrowser();
  if (browser)
    canClose = browser->GetHost()->TryCloseBrowser();

  // it's the end of the application lifecycle, reset callback to not push it
  // more than once
  if (canClose) {
    before_close_callback_.Reset();
  }
  return canClose;
}
...
CefSize ViewsWindow::GetMaximumSize(CefRefPtr<CefView> view) {
  CEF_REQUIRE_UI_THREAD();

  if (view->GetID() == ID_WINDOW)
    return maximum_window_size_;
...
ViewsWindow::ViewsWindow(Delegate* delegate,
  ...
  const bool show_overlays = hide_frame && !with_controls_ &&
