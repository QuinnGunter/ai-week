...
void BrowserWindowStdWin::CreateBrowser(
  ...
  extra_info = MainContext::Get()->GetGHybridDictionaryRepresentation();
...
void BrowserWindowStdWin::Undo() {
  if (client_handler_) {
      client_handler_->Undo();
  }
}

void BrowserWindowStdWin::Redo() {
  if (client_handler_) {
      client_handler_->Redo();
  }
}

void BrowserWindowStdWin::ShowDevTools() {
  if (client_handler_) {
...
void BrowserWindowStdWin::SetSize(size_t width, size_t height) {
  REQUIRE_MAIN_THREAD();

  HWND hwnd = GetWindowHandle();
  if (hwnd) {
    // Set the browser window size.
    SetWindowPos(hwnd, nullptr, 0, 0, static_cast<int>(width),
