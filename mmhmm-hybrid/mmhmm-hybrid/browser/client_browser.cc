...
class ClientBrowserDelegate : public ClientAppBrowser::Delegate {
 public:
  ...
  void OnContextInitialized(CefRefPtr<ClientAppBrowser> app) override {
    ...
    //register the custom handler for intercepting mmhmm-client and mmhmm-client-media requests
    client_request_handler::RegisterClientRequestHandlerFactory();
#if defined (OS_WIN)
    client_media_request_handler::RegisterClientMediaRequestHandlerFactory();
#endif
