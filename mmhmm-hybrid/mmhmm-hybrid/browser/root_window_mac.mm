...
#include "browser_window_std_mac.h"
...
class RootWindowMacImpl
 ...
 public:
  ...
  ClientWindowHandle GetParentRootWindowHandle() const;
...
ClientWindowHandle RootWindowMacImpl::GetParentRootWindowHandle() const {
  REQUIRE_MAIN_THREAD();
...
void RootWindowMacImpl::CreateRootWindow(const CefBrowserSettings& settings,
  ...
  // #75: Remember window size and location. Disabling this as it's absurd. Why would anyone want this?
  // Fix the window Y coordinate
  // [window_ setFrameTopLeftPoint:NSMakePoint(x, TransformY(y))];

  if (!is_popup_) {
    // Create the browser window.
    // #75: Remember window size and location. This was changed to reflect the contentBounds.size, rather than the default size of 800x600
    browser_window_->CreateBrowser(
        CAST_NSVIEW_TO_CEF_WINDOW_HANDLE(contentView),
        CefRect(0, 0, contentBounds.size.width, contentBounds.size.height), settings, nullptr,
...
//  #74: Always use application name as the window title
//  if (window_) {
...
void RootWindowMac::InitAsPopup(RootWindow::Delegate* delegate,
  ...
  DCHECK(delegate);
  delegate_ = delegate;
  impl_->InitAsPopup(delegate, with_controls, popupFeatures,
...
ClientWindowHandle RootWindowMac::GetParentRootWindowHandle() const {
  return impl_->GetParentRootWindowHandle();
}

void RootWindowMac::SetWindowTheme(WindowTheme theme) {
  // not implmented
}
