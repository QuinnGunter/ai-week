 ...
 */
JSONValue *JSONReader::Parse(const char *data, int dataLength)
{
	...
	JSONValue *value = JSONReader::Parse(w_data);
	free(w_data);
	return value;
}
 ...
 * @return JSONValue* Returns a JSON Value representing the root, or NULL on error
 */
JSONValue *JSONReader::Parse(const wchar_t *data)
{
 ...
 */
std::wstring JSONReader::Stringify(const JSONValue *value)
{
	if (value != NULL)
 ...
 */
bool JSONReader::SkipWhitespace(const wchar_t **data)
{
 ...
 */
bool JSONReader::ExtractString(const wchar_t **data, std::wstring &str)
{
	str = L"";
 ...
 */
double JSONReader::ParseInt(const wchar_t **data)
{
	double integer = 0;
 ...
 */
double JSONReader::ParseDecimal(const wchar_t **data)
{
	double decimal = 0.0;
  double factor = 0.1;
	while (**data != 0 && **data >= '0' && **data <= '9')
...
JSONValue *JSONValue::Parse(const wchar_t **data)
{
    ...
    {
        std::wstring str;
        if (!JSONReader::ExtractString(&(++(*data)), str))
            return NULL;
        else
    ...
    else if (**data == L'-' || (**data >= L'0' && **data <= L'9'))
    {
        ...
        if (**data == L'0')
            ...
            number = JSONReader::ParseInt(data);
        else
            return NULL;
        ...
        if (**data == '.')
        {
            ...
            double decimal = JSONReader::ParseDecimal(data);
        ...
        if (**data == L'E' || **data == L'e')
        {
            ...
            // Sort the expo out
            double expo = JSONReader::ParseInt(data);
    ...
    else if (**data == L'{')
    {
        ...
        while (**data != 0)
        {
            ...
            if (!JSONReader::SkipWhitespace(data))
            {
                FREE_OBJECT(object);
                return NULL;
            }
            ...
            std::wstring name;
            if (!JSONReader::ExtractString(&(++(*data)), name))
            {
                FREE_OBJECT(object);
                return NULL;
            }
...
*/
const JSONValue* JSONValue::Child(const wchar_t* name) const
{
...
const JSONValue *JSONValue::GetObjectJS( const wchar_t *name, bool * res ) const
{
    if( !this->HasChild(name) )          { *res=false; return nullptr; }
    if( !this->Child(name)->IsObject() ) { *res=false; return nullptr; }
                                           *res=true;  return this->Child(name);
}
const JSONArray *JSONValue::GetArray( const wchar_t *name, bool * res ) const
{
    if( !this->HasChild(name) )          { *res=false; return nullptr; }
    if( !this->Child(name)->IsArray() )  { *res=false; return nullptr; }
                                           *res=true;  return &this->Child(name)->AsArray();
}
const int JSONValue::GetInt( const wchar_t *name, bool * res ) const
{
    if( !this->HasChild(name) )          { *res=false; return 0; }
    if( !this->Child(name)->IsNumber() ) { *res=false; return 0; }
                                           *res=true;  return int(this->Child(name)->AsNumber());
}
const float JSONValue::GetFloat( const wchar_t *name, bool * res ) const
{
    if( !this->HasChild(name) )          { *res=false; return 0.0f; }
    if( !this->Child(name)->IsNumber() ) { *res=false; return 0.0f; }
                                           *res=true;  return float(this->Child(name)->AsNumber());
}
const bool JSONValue::GetBool( const wchar_t *name, bool * res ) const
{
    if( !this->HasChild(name) )          { *res=false; return false; }
    if( !this->Child(name)->IsBool() )   { *res=false; return false; }
                                           *res=true;  return this->Child(name)->AsBool();
}
const wchar_t * JSONValue::GetString( const wchar_t *name, bool * res ) const
{
    if( !this->HasChild(name) )          { *res=false; return nullptr; }
    if( !this->Child(name)->IsString() ) { *res=false; return nullptr; }
                                           *res=true;  return _wcsdup(this->Child(name)->AsString().c_str());
}

JSONValue *GetElementFromArray(const JSONArray *jsFilters, int id)
{
    return (JSONValue *)(*jsFilters)[id];
}