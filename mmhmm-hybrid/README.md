# Hey MMHMM folks: 
## The README below is from the CEF project, and while it's generally accurate, here are a few things you should know:
* The source code for our shell app is in the mmhmm-hybrid folder
* It's based on the cef_simple example (and there are still some lingering references)
* I added a few small bits of code, including:
* * adding void SimpleApp::OnBeforeCommandLineProcessing() to add the enable-media-stream command-line flag to the default set
* * adding an applicationDidBecomeActive: method to SimpleAppDelegate to request camera and microphone access
* * Adding Info.plist entries to the Resources/Info.plist, with the camera & mic permission prompts
* To get this up and running, follow the instructions below to create the .xcodeproj, then open that in Xcode, and select the "mmhmm-hybrid" target and run it.
* I have not tried this on Windows (or indeed, on an Intel Mac), but it works on my M1 PowerBook, at least as far as creating a call, and joining one.
