...
bool CopyD3D11TexToMem(
  ...
  if (texture_desc.Format != DXGI_FORMAT_NV12 &&
    ...
    DLOG(ERROR) << "Can't copy non-NV12 or BGRA texture. format="
                << static_cast<int>(texture_desc.Format);
    return false;
  }
  size_t copy_size = texture_desc.Format != DXGI_FORMAT_B8G8R8A8_UNORM
                         ? texture_desc.Height * texture_desc.Width * 3 / 2
                         : texture_desc.Height * texture_desc.Width * 4;
  ...
  return libyuv::NV12Copy(
    ...
    memcpy(dst_buffer.data(), source_buffer,
           texture_desc.Height * texture_desc.Width * 4);
    return true;
  }
}
