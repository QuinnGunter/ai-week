...
#include "content/browser/media/capture/screen_capture_kit_config_mac.h"  // MMHMM
...
+ (SCStreamConfiguration*)streamConfigurationWithFrameSize:(gfx::Size)frameSize
                                           ...
                                           customConfigJson:(const std::string&)config_json {  // MMHMM
  ...
  // MMHMM START: Apply custom configuration
  if (!config_json.empty()) {
    NSDictionary* custom_config = content::DeserializeStreamConfiguration(config_json);
    if (content::HasStreamConfigOverrides(custom_config)) {
      content::ApplyConfigurationToStream(config, custom_config);
      DLOG(INFO) << "Applied custom stream configuration to SCStreamConfiguration";
    }
  }
  // MMHMM END
...
class API_AVAILABLE(macos(12.3)) ScreenCaptureKitDeviceMac
 ...
 public:
  // MMHMM: Added config_json parameter
  ...
  void OnShareableContentCreated(SCShareableContent* content) {
    ...
    // MMHMM: Check if config contains content filter data
    SCContentFilter* filter = nullptr;
    if (!config_json_.empty()) {
      NSDictionary* custom_config = content::DeserializeStreamConfiguration(config_json_);
      if (content::HasContentFilterOverrides(custom_config)) {
        filter = content::CreateContentFilterFromConfig(custom_config, content);
        if (filter) {
          DLOG(INFO) << "Using content filter from JSON config";
          CreateStream(filter);
          return;
        }
      }
  ...
  void CreateStream(SCContentFilter* filter) {
    ...
    SCStreamConfiguration* config = [ScreenCaptureKitDeviceHelper
        streamConfigurationWithFrameSize:actual_capture_format_.frame_size
                         ...
                         customConfigJson:config_json_];  // MMHMM
 ...
 private:
  ...
  const std::string config_json_;  // MMHMM
...
// MMHMM: Overload 3 - Full signature with config_json parameter
...
std::unique_ptr<media::VideoCaptureDevice> CreateScreenCaptureKitDeviceMac(
  ...
  // MMHMM: Log custom config if present
  if (!config_json.empty()) {
    DLOG(INFO) << "Creating ScreenCaptureKitDevice with custom config: " 
               << config_json;
  }
  ...
  return std::make_unique<ScreenCaptureKitDeviceMac>(source, config_json, filter);
}
