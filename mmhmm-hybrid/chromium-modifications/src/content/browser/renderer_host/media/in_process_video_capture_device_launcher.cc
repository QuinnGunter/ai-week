...
// MMHMM: Added stream_config_json parameter
DesktopCaptureImplementation CreatePlatformDependentVideoCaptureDevice(
    ...
    const std::string& stream_config_json,
  ...
  if (desktop_id.type == DesktopMediaID::TYPE_WINDOW ||
    ...
    // MMHMM: Pass config to device creation
    device_out = CreateScreenCaptureKitDeviceMac(desktop_id, stream_config_json);
    if (device_out) {
      return kScreenCaptureKitDeviceMac;
    }
  }
...
void InProcessVideoCaptureDeviceLauncher::DoStartDesktopCaptureOnDeviceThread(
  ...
  // MMHMM: Log stream configuration if present
  if (!params.stream_configuration_json.empty()) {
    LOG(INFO) << "Launching device with custom stream configuration.";
  } else {
    LOG(INFO) << "Launching device with default stream configuration.";
  }
  ...
  DesktopCaptureImplementation implementation =
      CreatePlatformDependentVideoCaptureDevice(
          ...
          params.stream_configuration_json, video_capture_device);
