...
VideoCaptureManager::GetOrCreateController(
  ...
  // MMHMM: Copy stream configuration from device to params
  media::VideoCaptureParams params_with_config = params;
  params_with_config.stream_configuration_json = device_info.stream_configuration_json;
  
  scoped_refptr<VideoCaptureController> new_controller =
      base::MakeRefCounted<VideoCaptureController>(
          device_info.id, device_info.type, params_with_config,
