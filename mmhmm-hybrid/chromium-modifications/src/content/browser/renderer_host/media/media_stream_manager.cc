...
void MediaStreamManager::HandleAccessRequestResponse(
  ...
  for (size_t stream_index = 0;
    ...
    for (const std::optional<blink::MediaStreamDevice>* device_ptr :
      ...
      // MMHMM: Copy stream configuration from request controls to device
      if (device.type == request->video_type()) {
        device.stream_configuration_json = 
            request->stream_controls().video.stream_configuration_json;
      }
