...
bool StructTraits<blink::mojom::MediaStreamDeviceDataView,
  ...
  // MMHMM: Read and validate stream_configuration_json
  if (!input.ReadStreamConfigurationJson(&out->stream_configuration_json)) {
    return false;
  }
  if (out->stream_configuration_json.size() > 8192) {
    return false;  // Reject oversized configuration (max 8KB)
  }
  return true;
}
