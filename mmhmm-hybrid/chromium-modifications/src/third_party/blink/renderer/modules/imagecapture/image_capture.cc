...
using SegmentationMode = media::mojom::blink::SegmentationMode;
using BlurMode = media::mojom::blink::BlurMode;
...
void CopyCommonMembers(const T* source,
  ...
  if (source->hasSegmentationMode()) {
    destination->setSegmentationMode(source->segmentationMode());
  }
  if (source->hasBlurMode()) {
    destination->setBlurMode(source->blurMode());
  }
}
...
SegmentationMode ParseSegmentationMode(const String& mode) {
  if (mode == "none")
    return SegmentationMode::NONE;
  if (mode == "silhouette")
    return SegmentationMode::SILHOUETTE;
  if (mode == "circle")
    return SegmentationMode::CIRCLE;
  NOTREACHED();
}

BlurMode ParseBlurMode(const String& mode) {
  if (mode == "none")
    return BlurMode::NONE;
  if (mode == "light")
    return BlurMode::LIGHT;
  if (mode == "strong")
    return BlurMode::STRONG;
  NOTREACHED();
}
...
WebString ToString(SegmentationMode value) {
  switch (value) {
    case SegmentationMode::NONE:
      return "none";
    case SegmentationMode::SILHOUETTE:
      return "silhouette";
    case SegmentationMode::CIRCLE:
      return "circle";
  }
}
      
WebString ToString(BlurMode value) {
  switch (value) {
    case BlurMode::NONE:
      return "none";
    case BlurMode::LIGHT:
      return "light";
    case BlurMode::STRONG:
      return "strong";
  }
}
...
Vector<String> ApplyExactValueConstraint(
    bool* has_setting_ptr,
    SegmentationMode* setting_ptr,
    const Vector<String>& effective_capability,
    const String& exact_constraint) {
  // Update the setting.
  *has_setting_ptr = true;
  *setting_ptr = ParseSegmentationMode(exact_constraint);
  // Update the effective capability.
  return {exact_constraint};
}
...
Vector<String> ApplyExactValueConstraint(
  ...
  if (!*has_setting_ptr ||
      !base::Contains(exact_constraints,
                      static_cast<const String&>(ToString(*setting_ptr)))) {
    *has_setting_ptr = true;
    *setting_ptr = ParseSegmentationMode(new_effective_capability[0]);
  }
  return new_effective_capability;
}
...
Vector<String> ApplyExactValueConstraint(
    bool* has_setting_ptr,
    BlurMode* setting_ptr,
    const Vector<String>& effective_capability,
    const String& exact_constraint) {
  // Update the setting.
  *has_setting_ptr = true;
  *setting_ptr = ParseBlurMode(exact_constraint);
  // Update the effective capability.
  return {exact_constraint};
}
...
Vector<String> ApplyExactValueConstraint(
  ...
  if (!*has_setting_ptr ||
      !base::Contains(exact_constraints,
                      static_cast<const String&>(ToString(*setting_ptr)))) {
    *has_setting_ptr = true;
    *setting_ptr = ParseBlurMode(new_effective_capability[0]);
  }
  return new_effective_capability;
}
...
Vector<String> ApplyIdealValueConstraint(
  ...
  // Validate and update the setting.
  *has_setting_ptr = true;
  *setting_ptr = ParseSegmentationMode(
      base::Contains(effective_capability, ideal_constraint) ? ideal_constraint
                                                             : current_setting);
  // Keep the effective capability intact.
  return effective_capability;
...
Vector<String> ApplyIdealValueConstraint(
  ...
  if (!*has_setting_ptr ||
    ...
    *has_setting_ptr = true;
    *setting_ptr = ParseSegmentationMode(setting_name);
  }
  // Keep the effective capability intact.
  return effective_capability;
}
  ...
  Vector<String> ApplyIdealValueConstraint(
    ...
    if (!*has_setting_ptr ||
      ...
      *has_setting_ptr = true;
      *setting_ptr = ParseBlurMode(setting_name);
    }
    // Keep the effective capability intact.
    return effective_capability;
  }
...
void ImageCapture::ApplyMediaTrackConstraintSetToSettings(
  ...
  if (constraint_set->hasSegmentationMode() &&
      effective_capabilities->hasSegmentationMode()) {
    effective_capabilities->setSegmentationMode(ApplyValueConstraint(
        &settings->has_segmentation_mode, &settings->segmentation_mode,
        effective_capabilities->segmentationMode(), constraint_set->segmentationMode(),
        constraint_set_type, settings_->segmentationMode()));
  }
      
  if (constraint_set->hasBlurMode() &&
      effective_capabilities->hasBlurMode()) {
    effective_capabilities->setBlurMode(ApplyValueConstraint(
        &settings->has_blur_mode, &settings->blur_mode,
        effective_capabilities->blurMode(), constraint_set->blurMode(),
        constraint_set_type, settings_->blurMode()));
  }
}
...
void ImageCapture::UpdateMediaTrackSettingsAndCapabilities(
  ...
  WTF::Vector<WTF::String> supported_segmentation_modes;
  supported_segmentation_modes.ReserveInitialCapacity(
      photo_state->supported_segmentation_modes.size());
  for (const auto& supported_mode : photo_state->supported_segmentation_modes) {
    supported_segmentation_modes.push_back(ToString(supported_mode));
  }
  if (!supported_segmentation_modes.empty()) {
    capabilities_->setSegmentationMode(std::move(supported_segmentation_modes));
    settings_->setSegmentationMode(
        ToString(photo_state->current_segmentation_mode));
  }

  WTF::Vector<WTF::String> supported_blur_modes;
  supported_blur_modes.ReserveInitialCapacity(
      photo_state->supported_blur_modes.size());
  for (const auto& supported_mode : photo_state->supported_blur_modes) {
    supported_blur_modes.push_back(ToString(supported_mode));
  }
  if (!supported_blur_modes.empty()) {
    capabilities_->setBlurMode(std::move(supported_blur_modes));
    settings_->setBlurMode(
        ToString(photo_state->current_blur_mode));
  }
...
ImageCapture::GetConstraintWithCapabilityExistenceMismatch(
  ...
  if (constraint_set->hasSegmentationMode() &&
      !CheckIfCapabilityExistenceSatisfiesConstraint(
          constraint_set->segmentationMode(),
          CapabilityExists(capabilities_->hasSegmentationMode()),
          constraint_set_type)) {
    return "segmentationMode";
  }
  if (constraint_set->hasBlurMode() &&
      !CheckIfCapabilityExistenceSatisfiesConstraint(
          constraint_set->blurMode(),
          CapabilityExists(capabilities_->hasBlurMode()),
          constraint_set_type)) {
    return "blurMode";
  }
  return std::nullopt;
