...
// MMHMM: Helper to extract stream configuration from constraint set
std::string ExtractStreamConfig(
    const MediaTrackConstraintSetPlatform& constraint) {
  if (constraint.mmhmm_stream_configuration.HasExact()) {
    const Vector<String>& config_vec =
        constraint.mmhmm_stream_configuration.Exact();
    if (!config_vec.empty()) {
      return config_vec[0].Utf8();
    }
  }
  return "";
}
...
void UserMediaProcessor::SelectVideoContentSettings() {
  ...
  // MMHMM: Extract stream configuration from constraints
  // Try basic constraints first
  std::string stream_config = ExtractStreamConfig(
      current_request_info_->request()->VideoConstraints().Basic());
  
  // If not found in basic, check advanced constraints
  if (stream_config.empty()) {
    const auto& advanced_vec =
        current_request_info_->request()->VideoConstraints().Advanced();
    for (const auto& advanced_element : advanced_vec) {
      stream_config = ExtractStreamConfig(advanced_element);
      if (!stream_config.empty()) {
        break;  // Use first matching advanced constraint
      }
    }
  }
  ...
  if (!stream_config.empty()) {
    current_request_info_->stream_controls()->video.stream_configuration_json =
        stream_config;
    DLOG(INFO) << "Setting custom stream configuration.";
  }
