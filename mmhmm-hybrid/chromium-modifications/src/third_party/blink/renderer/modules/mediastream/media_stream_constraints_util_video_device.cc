    ...
    for (auto& format : device.formats) {
      ...
      for (auto& noise_reduction : capabilities.noise_reduction_capabilities) {
        ...
        if (std::lexicographical_compare(candidate_distance_vector.begin(),
          ...
          capture_params.enable_segmentation =
              constraints.Basic().segmentation_mode.HasExact() ||
              constraints.Basic().segmentation_mode.HasIdeal() ||
              constraints.Basic().segmentation_mode.HasMandatory();
