...
bool VideoCaptureImpl::BindVideoFrameOnMediaTaskRunner(
  ...
  DCHECK(video_frame_init_data.ready_buffer->info->pixel_format == media::PIXEL_FORMAT_NV12 ||
    video_frame_init_data.ready_buffer->info->pixel_format == media::PIXEL_FORMAT_NV12A ||
    video_frame_init_data.ready_buffer->info->pixel_format == media::PIXEL_FORMAT_BGRA);
  ...
  DCHECK(
      ...
      output_format ==
          media::GpuVideoAcceleratorFactories::OutputFormat::BGRA);
  ...
  if (!shared_image || should_recreate_shared_image) {
    ...
    auto multiplanar_si_format = viz::MultiPlaneFormat::kNV12;
      ...
      switch (video_frame_init_data.ready_buffer->info->pixel_format){
        case media::PIXEL_FORMAT_BGRA:
          multiplanar_si_format = viz::SinglePlaneFormat::kBGRA_8888;
          break;
        case media::PIXEL_FORMAT_NV12A:
          multiplanar_si_format = viz::MultiPlaneFormat::kNV12A;
          break;
        default:
        multiplanar_si_format = viz::MultiPlaneFormat::kNV12;
      }
    ...
    // Chromium assumes all alpha carrying formats have premultiplied alpha by default.
    // Ensure our alpha carrying camera frames indicate they are unpremultiplied
    gpu::SharedImageInfo si_info(
        multiplanar_si_format, size, color_space,
        kTopLeft_GrSurfaceOrigin,
        video_frame_init_data.ready_buffer->info->pixel_format ==
                media::PIXEL_FORMAT_BGRA
