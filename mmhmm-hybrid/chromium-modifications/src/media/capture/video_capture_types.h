...
enum class VideoCaptureError {
  ...
  kWinMediaFoundationMmhmmDeviceInitializationFailed = 151,
  kMaxValue = 151
};
...
struct CAPTURE_EXPORT VideoCaptureParams {
  ...
  bool operator==(const VideoCaptureParams& other) const {
    return requested_format == other.requested_format &&
           ...
           enable_segmentation == other.enable_segmentation &&
           stream_configuration_json == other.stream_configuration_json;
  }
  ...
  // User-specified flag indicating if segmentation should be enabled.
  ...
  // MMHMM START: Custom stream configuration for Airtime
  // JSON config passed from Swift to control SCStreamConfiguration/SCContentFilter
  // See: mmhmm-hybrid.wiki/Screen-Share-Configuration.md
  std::string stream_configuration_json;
  // MMHMM END
};
