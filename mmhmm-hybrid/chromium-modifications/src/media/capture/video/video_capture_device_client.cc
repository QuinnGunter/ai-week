...
bool IsFormatSupported(media::VideoPixelFormat pixel_format) {
  return (pixel_format == media::PIXEL_FORMAT_I420 ||
          ...
          pixel_format == media::PIXEL_FORMAT_BGRA ||
...
void GetI420ABufferAccess(
    const media::VideoCaptureDevice::Client::Buffer& buffer,
    const gfx::Size& dimensions,
    uint8_t** y_plane_data,
    uint8_t** u_plane_data,
    uint8_t** v_plane_data,
    uint8_t** a_plane_data,
    int* y_plane_stride,
    int* uv_plane_stride) {
  *y_plane_data = buffer.handle_provider->GetHandleForInProcessAccess()->data();
  *u_plane_data = *y_plane_data + media::VideoFrame::PlaneSize(
  ...
  *v_plane_data = *u_plane_data + media::VideoFrame::PlaneSize(
                                      ...
                                      media::VideoFrame::Plane::kU, dimensions)
                                      .GetArea();
  *a_plane_data = *v_plane_data + media::VideoFrame::PlaneSize(
                                      media::PIXEL_FORMAT_I420A,
...
void VideoCaptureDeviceClient::OnIncomingCapturedData(
  ...
  auto outgoing_format =
      (alpha && alpha_length > 0) ? PIXEL_FORMAT_I420A : PIXEL_FORMAT_I420;

  // The input |length| can be greater than the required buffer size because of
  // paddings and/or alignments, but it cannot be smaller.
  CHECK_GE(static_cast<size_t>(length + alpha_length),
  ...
  auto reservation_result_code = ReserveOutputBuffer(
      dimensions, outgoing_format, frame_feedback_id, &buffer,
  ...
  uint8_t* a_plane_data = nullptr;
  int yplane_stride, uv_plane_stride = 0;

  if (alpha && alpha_length > 0) {
    GetI420ABufferAccess(buffer, dimensions, &y_plane_data, &u_plane_data,
                         &v_plane_data, &a_plane_data, &yplane_stride,
                         &uv_plane_stride);
  } else {
  ...
  const gfx::ColorSpace color_space =
      OverrideColorSpaceForLibYuvConversion(data_color_space, outgoing_format);
  ...
  if (alpha && alpha_length > 0) {
    memcpy(a_plane_data, alpha, alpha_length);
  }

  const VideoCaptureFormat output_format =
      VideoCaptureFormat(dimensions, format.frame_rate, outgoing_format);
...
bool VideoCaptureDeviceClient::ConvertAllToI420(
    const uint8_t* src,
    int src_length,
    uint8_t* destination,
    int rotation,
  ...
  GetI420BufferAccess(destination, dimensions, &y_plane_data, &u_plane_data,
                        &v_plane_data, &yplane_stride, &uv_plane_stride);
...
// see https://msdn.microsoft.com/en-us/library/windows/desktop/dd407253
  ...
  if (libyuv::ConvertToI420(
          src, src_length, y_plane_data, yplane_stride, u_plane_data,
