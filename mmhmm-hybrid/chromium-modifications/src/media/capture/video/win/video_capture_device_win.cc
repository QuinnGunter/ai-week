...
void VideoCaptureDeviceWin::GetPhotoState(GetPhotoStateCallback callback) {
  ...
  //RJ : Moved this to prevent an early exit. Other photo state
  //is reliant on external controls. Segmentation is not.
...
void VideoCaptureDeviceWin::SetPhotoOptions(
  ...
  //RJ : Set segmentation mode regardless of other controls
...
void VideoCaptureDeviceWin::FrameReceived(const uint8_t* buffer,
                                          ...
                                          bool flip_y) {
  const mmhmm::ScopedSegmentorFrameProcTimeUpdater last_frame_duration_counter{segmentor_};
...

  {
    ...
    VideoCaptureFormat current_format = format;
    ...
    if (format.pixel_format != PIXEL_FORMAT_I420) {
      ...
      if (!client_->ConvertAllToI420(buffer, length,
      ...
      frame_buffer_length = length;
    }
    current_format.pixel_format = PIXEL_FORMAT_I420A;
    segmentor_.SegmentFrame(frame_buffer, current_format.frame_size.width(),
                            current_format.frame_size.height(),
                            current_format.pixel_format, segmentation_mode_,
                            mmhmm::Segmentor::MaskType::uint8_mask);

    if (segmentation_mode_ == media::mojom::SegmentationMode::SILHOUETTE) {

      auto mask = segmentor_.GetUint8Mask();
      CHECK(mask.size() == static_cast<std::size_t>(current_format.frame_size.width()) *
                               current_format.frame_size.height());

      client_->OnIncomingCapturedData(
          frame_buffer, frame_buffer_length, current_format, gfx::ColorSpace(),
          camera_rotation_.value(), false /* flip_y */, base::TimeTicks::Now(),
          timestamp, std::nullopt,/*metadata=*/std::nullopt, 0, mask.data(), mask.size());
    } else {
      ...
      // DXVA_NominalRangeto build a gfx::ColorSpace. See
      // http://crbug.com/959992.
      auto mask = segmentor_.GetOpaqueMaskRef();
      CHECK(mask.size() ==
            static_cast<std::size_t>(current_format.frame_size.width() *
                                     current_format.frame_size.height()));
      client_->OnIncomingCapturedData(
          frame_buffer, frame_buffer_length, current_format, gfx::ColorSpace(),
          camera_rotation_.value(), false /* flip_y */, base::TimeTicks::Now(),
