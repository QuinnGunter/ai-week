...
HRESULT CreateSharedHandleFromCaptureTexture(ID3D11Texture2D* texture,
                                             HANDLE* dxgi_handle) {
  IDXGIResource1* dxgiResource = nullptr;
  // Query the texture for the IDXGIResource interface
  HRESULT hr =
  ...
  if (SUCCEEDED(hr)) {
    hr = dxgiResource->CreateSharedHandle(
        ...
        nullptr, dxgi_handle);
    if (FAILED(hr)) {
      LOG(ERROR) << logging::SystemErrorCodeToString(hr);
      return hr;
    }
...
HRESULT CheckOrCreateSharedTextureFromTexture(
    ID3D11Texture2D* inputTexture,
    Microsoft::WRL::ComPtr<ID3D11Texture2D>& outputTexture,
    HANDLE* dxgi_handle) {
  if (!inputTexture || !dxgi_handle) {
    return E_INVALIDARG;
  }

  // Get the device from the input texture
  Microsoft::WRL::ComPtr<ID3D11Device> device;
  inputTexture->GetDevice(&device);
  if (!device) {
    return E_FAIL;
  }

  // Get the description from the input texture
  D3D11_TEXTURE2D_DESC inputDesc;
  inputTexture->GetDesc(&inputDesc);

  // Check if output texture already exists and is valid
  if (outputTexture) {
    // Get device from output texture
    Microsoft::WRL::ComPtr<ID3D11Device> outputDevice;
    outputTexture->GetDevice(&outputDevice);

    // Check if devices match
    if (outputDevice.Get() == device.Get()) {
      // Get description from output texture
      D3D11_TEXTURE2D_DESC outputDesc;
      outputTexture->GetDesc(&outputDesc);

      // Check if dimensions and format match, and has required flags
      bool dimensionsMatch = (outputDesc.Width == inputDesc.Width &&
                              outputDesc.Height == inputDesc.Height);
      bool formatMatches = (outputDesc.Format == inputDesc.Format);
      bool hasSharedFlags =
          (outputDesc.MiscFlags & D3D11_RESOURCE_MISC_SHARED_NTHANDLE) &&
          (outputDesc.MiscFlags & D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX);

      if (dimensionsMatch && formatMatches && hasSharedFlags) {
        return S_OK;  // Texture is valid
      }
    }
    // If we reach here, existing texture is not compatible, so we'll create a
    // new one
    outputTexture.Reset();
  }

  // Modify the description to enable shared resource creation with keyed mutex
  ...
  // Query for IDXGIResource1 interface to create shared handle
  Microsoft::WRL::ComPtr<IDXGIResource1> dxgiResource;
  hr = outputTexture.As(&dxgiResource);
  if (FAILED(hr)) {
    LOG(ERROR) << "Failed to query IDXGIResource1: "
               << logging::SystemErrorCodeToString(hr);
    return hr;
  }
  ...
  hr = dxgiResource->CreateSharedHandle(
      ...
      dxgi_handle);
  if (FAILED(hr)) {
    LOG(ERROR) << "Failed to create shared handle: "
               << logging::SystemErrorCodeToString(hr);
    return hr;
  }
...
void VideoCaptureDeviceMFWin::AllocateAndStart(
  ...
  // Disable hardware accel if using segmentation and we are not going to use
  // accelerated pipeline. Copy back is too costly
  if (params.enable_segmentation && use_dx_seglib_ == false) {
...
void VideoCaptureDeviceMFWin::GetPhotoState(GetPhotoStateCallback callback) {
  ...
  photo_capabilities->current_blur_mode = media::mojom::BlurMode::NONE;
  photo_capabilities->supported_blur_modes =
      std::vector<media::mojom::BlurMode>(
          {media::mojom::BlurMode::NONE,
           media::mojom::BlurMode::LIGHT,
           media::mojom::BlurMode::STRONG});
  
  photo_capabilities->current_segmentation_mode = segmentation_mode_;
  photo_capabilities->supported_segmentation_modes =
      std::vector<media::mojom::SegmentationMode>(
          {media::mojom::SegmentationMode::NONE,
           media::mojom::SegmentationMode::SILHOUETTE,
           media::mojom::SegmentationMode::CIRCLE});
...
void VideoCaptureDeviceMFWin::SetPhotoOptions(
  ...
  //Segmentation is applied internally so set regardless of state or
  //camera's supported capabilities
  if (settings->has_segmentation_mode) {
    segmentation_mode_ = settings->segmentation_mode;
    gpu_segmentor_.SetSegmentationMode(segmentation_mode_);
  }

  if (settings->has_blur_mode) {
    gpu_segmentor_.SetBlurMode(settings->blur_mode);
  }
...
HRESULT VideoCaptureDeviceMFWin::DeliverTextureToClient(
  ...
  if (use_dx_seglib_) {
    // Change pixel format here because frames will be pushed in BGRA
  ...
  if (use_dx_seglib_) {
    HANDLE capture_texture_handle = nullptr;

    // Expected that in 99.99% cases incoming texutre is already cross process
    // enabled.
    if (is_cross_process_shared_texture) {
      hr = CreateSharedHandleFromCaptureTexture(texture,
                                                &capture_texture_handle);

      if (FAILED(hr)) {
        return MF_E_UNEXPECTED;
      }
    } else {
      // For any unexpected case we will use staging texture with additional
      // GPUtoGPU copy
      hr = CheckOrCreateSharedTextureFromTexture(
          texture, staging_shared_texture, &staging_shared_handle);

      if (FAILED(hr)) {
        return MF_E_UNEXPECTED;
      }

      hr = CopyTextureToGpuMemoryBuffer(texture, staging_shared_handle);
      
      if (FAILED(hr)) {
        return MF_E_UNEXPECTED;
      }

      capture_texture_handle = staging_shared_handle;
    }

    gpu_segmentor_.SegmentFrame(capture_texture_handle,
...
void VideoCaptureDeviceMFWin::OnIncomingCapturedDataInternal() {
  ...
  const mmhmm::ScopedSegmentorFrameProcTimeUpdater last_frame_duration_counter{cpu_segmentor_, gpu_segmentor_};
  ...
  if (!delivered_texture && client_.get()) {
    VideoCaptureFormat format = selected_video_capability_->supported_format;
    uint8_t* frame_buffer = nullptr;
    int frame_buffer_length = 0;

    if (format.pixel_format != PIXEL_FORMAT_I420) {
      int color_convert_buffer_length =
          format.frame_size.width() * format.frame_size.height() * 1.5;
      color_convert_buffer_.resize(color_convert_buffer_length, 0);

      if (!client_->ConvertAllToI420(locked_buffer.data(),
                                     locked_buffer.length(),
                                     color_convert_buffer_.data(),
                                     camera_rotation_.value(), false, format)) {
        LOG(ERROR) << "Unable to convert frame to I420";
        return;
      }

      frame_buffer = color_convert_buffer_.data();
      frame_buffer_length = color_convert_buffer_length;
    }
    else {
      frame_buffer = locked_buffer.data();
      frame_buffer_length = locked_buffer.length();
    }

    format.pixel_format = PIXEL_FORMAT_I420A;

    cpu_segmentor_.SegmentFrame(frame_buffer, format.frame_size.width(),
                            format.frame_size.height(), format.pixel_format,
                            segmentation_mode_,
                            mmhmm::Segmentor::MaskType::uint8_mask);

    if (segmentation_mode_ == media::mojom::SegmentationMode::SILHOUETTE) {
      auto mask = cpu_segmentor_.GetUint8Mask();
      CHECK(mask.size() == static_cast<std::size_t>(format.frame_size.width()) *
                               format.frame_size.height());

    client_->OnIncomingCapturedData(
          frame_buffer, frame_buffer_length, format, color_space_,
          camera_rotation_.value(), false /* flip_y */, reference_time,
          timestamp, MaybeForwardCaptureBeginTime(capture_begin_time),/*metadata=*/std::nullopt, 0, mask.data(), mask.size());
    } else {
      auto mask = cpu_segmentor_.GetOpaqueMaskRef();
      CHECK(mask.size() == static_cast<std::size_t>(format.frame_size.width()) *
                               format.frame_size.height());
      client_->OnIncomingCapturedData(
          frame_buffer, frame_buffer_length, format,
          gfx::ColorSpace(), camera_rotation_.value(), false /* flip_y */,
          reference_time, timestamp, MaybeForwardCaptureBeginTime(capture_begin_time),/*metadata=*/std::nullopt, 0, mask.data(), mask.size());
  }
  }
