...
base::win::ScopedHandle CreateTexture(ID3D11Device* d3d11_device,
                                      const gfx::Size& size,
...
bool GpuMemoryBufferTrackerWin::Init(const gfx::Size& dimensions,
  ...
  // Only support NV12 and BGRA
  if (format != PIXEL_FORMAT_NV12 && format != PIXEL_FORMAT_BGRA) {
  ...
  if (is_external_dxgi_handle_) {
    return CreateBufferInternal(std::move(external_dxgi_handle_),
                                std::move(dimensions), format);
  }

  auto dxgi_format =
      format == PIXEL_FORMAT_NV12 ? DXGI_FORMAT_NV12 : DXGI_FORMAT_B8G8R8A8_UNORM;
  base::win::ScopedHandle scoped_handle =
      CreateTexture(d3d_device_.Get(), dimensions, dxgi_format);
...
bool GpuMemoryBufferTrackerWin::CreateBufferInternal(
  ...
  stride_ = gfx::RowSizeForBufferFormat(dimensions_.width(),
                                        format == PIXEL_FORMAT_NV12 ? gfx::BufferFormat::YUV_420_BIPLANAR : gfx::BufferFormat::BGRA_8888,
                                        /*plane=*/0);
...
bool GpuMemoryBufferTrackerWin::IsReusableForFormat(
  ...
  return !IsD3DDeviceChanged() && (format == PIXEL_FORMAT_NV12 || format == PIXEL_FORMAT_BGRA) &&
