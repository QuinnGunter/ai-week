...
class CAPTURE_EXPORT VideoCaptureDeviceMFWin : public VideoCaptureDevice {
 ...
 private:
  ...
  int num_restarts_ = 0;
  std::atomic<int> frames_awaiting_processing_ = 0;
  ...
  // Segmentation
  media::mojom::SegmentationMode segmentation_mode_ =
      media::mojom::SegmentationMode::NONE;
  mmhmm::Segmentor cpu_segmentor_{mmhmm::Segmentor::PipelineVersion::cpu};
  mmhmm::Segmentor gpu_segmentor_{mmhmm::Segmentor::PipelineVersion::directX};

  // For rare case (personally never encountered), when MediaFoundation will
  // deliver not cross process texture, we need to copy gpu data into staging
  // cross process texture as seglib is accepting only cross process shared gpu
  // buffers.
  Microsoft::WRL::ComPtr<ID3D11Texture2D> staging_shared_texture;
  HANDLE staging_shared_handle = nullptr;
  bool use_dx_seglib_ = true;

  std::vector<uint8_t> color_convert_buffer_;
