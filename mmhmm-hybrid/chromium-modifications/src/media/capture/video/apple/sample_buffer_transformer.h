...
class CAPTURE_EXPORT SampleBufferTransformer {
 public:
  ...
  std::span<const std::uint8_t> PerformSegmentation(CVPixelBufferRef pixel_buffer, gfx::Size destination_size);
 ...
 private:
  ...
  std::unique_ptr<PixelBufferPool> staging_pixel_buffer_pool_;
  ...
  // segmentation
  mmhmm::Segmentor segmentor_ {mmhmm::Segmentor::PipelineVersion::cpu};
  std::atomic<media::mojom::SegmentationMode> segmentation_mode_{
