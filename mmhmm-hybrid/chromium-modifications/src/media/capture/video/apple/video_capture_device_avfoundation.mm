...
#include <dlfcn.h>
#include "media/capture/video/segmentor.h"
...
@implementation VideoCaptureDeviceAVFoundation {
  ...
  std::unique_ptr<mmhmm::Segmentor> _segmentor;
  std::unique_ptr<media::PixelBufferPool> _outputPixelBufferPool;
  CGSize _currentPixelBufferPoolsDimensions;
...
- (instancetype)initWithFrameReceiver:
  ...
  if ((self = [super init])) {
    ...
    _segmentor = std::make_unique<mmhmm::Segmentor>(mmhmm::Segmentor::PipelineVersion::metal);
    CHECK(_segmentor);
#if BUILDFLAG(IS_IOS)
...
- (void)setSegmentationMode:(media::mojom::SegmentationMode)segmentationMode {
  if (_sampleBufferTransformer) {
    _sampleBufferTransformer->SetSegmentationMode(segmentationMode);
  }
  
  _segmentor->SetSegmentationMode(segmentationMode);
}

- (void)setBlurMode:(media::mojom::BlurMode)blurMode {
  _segmentor->SetBlurMode(blurMode);
}
...
- (BOOL)setCaptureHeight:(int)height
  ...
  if (_bestCaptureFormat) {
    best_fourcc = CMFormatDescriptionGetMediaSubType(_bestCaptureFormat.formatDescription);
  }
...
#endif
  if (_currentPixelBufferPoolsDimensions.width != width || _currentPixelBufferPoolsDimensions.height != height) {
    _outputPixelBufferPool = media::PixelBufferPool::Create(
                                                            kCVPixelFormatType_32BGRA, width, height, kPixelBufferPoolSize);
  }
  return YES;
}
...
- (BOOL)processSampleBufferRGBAIOSurface:(CMSampleBufferRef)sampleBuffer
                               timestamp:(const base::TimeDelta)timestamp
                      capture_begin_time:(std::optional<base::TimeTicks>)capture_begin_time {
  VLOG(3) << __func__;
  if (!_segmentor || !_outputPixelBufferPool) {
    return NO;
  }
  
  _lock.AssertAcquired();
  
  CVImageBufferRef cameraBuffer = CMSampleBufferGetImageBuffer(sampleBuffer);

  if (!cameraBuffer || CFGetTypeID(cameraBuffer) != CVPixelBufferGetTypeID()) {
    return NO;
  }
  
  CVPixelBufferRef inputBuffer = (CVPixelBufferRef)cameraBuffer;

  base::apple::ScopedCFTypeRef<CVPixelBufferRef> outputBuffer = _outputPixelBufferPool->CreateBuffer();

  if (!outputBuffer) {
    return NO;
  }
  
  if (_segmentor->SegmentFrame(static_cast<void*>(inputBuffer), static_cast<void*>(outputBuffer.get())) != 0) {
    return NO;
  }
  
  // Extract destination IOSurface
  IOSurfaceRef destinationSurface = CVPixelBufferGetIOSurface(outputBuffer.get());
  if (!destinationSurface) {
    return NO;
  }

  gfx::Size frameSize(CVPixelBufferGetWidth(inputBuffer), CVPixelBufferGetHeight(inputBuffer));
  
  const media::VideoCaptureFormat captureFormat(frameSize, _frameRate, media::PIXEL_FORMAT_BGRA);
  
  media::CapturedExternalVideoBuffer externalBuffer = [self capturedExternalVideoBufferFromIOSurface:destinationSurface captureFormat:captureFormat colorSpace:kColorSpaceRec709Apple];

  DCHECK(_frameReceiver);
  _frameReceiver->ReceiveExternalGpuMemoryBufferFrame(std::move(externalBuffer), timestamp, capture_begin_time);
  return YES;
}
...
- (void)processPixelBufferNV12IOSurface:(CVPixelBufferRef)pixelBuffer
                     ...
                     capture_begin_time:
                         (std::optional<base::TimeTicks>)capture_begin_time {
  VLOG(3) << __func__;
  DCHECK_EQ(captureFormat.pixel_format, media::PIXEL_FORMAT_NV12A);

  IOSurfaceRef ioSurface = CVPixelBufferGetIOSurface(pixelBuffer);
  DCHECK(ioSurface);
  media::CapturedExternalVideoBuffer externalBuffer =
      [self capturedExternalVideoBufferFromIOSurface:ioSurface
...
- (media::CapturedExternalVideoBuffer)
    capturedExternalVideoBufferFromIOSurface:(IOSurfaceRef)ioSurface
                                   captureFormat:
...
- (void)captureOutput:(AVCaptureOutput*)captureOutput
  ...
  if (_useGPUMemoryBuffer && sampleHasPixelBufferOrIsMjpeg) {
    if ([self processSampleBufferRGBAIOSurface:sampleBuffer
                                    timestamp:timestamp
    ...
    const media::VideoCaptureFormat captureFormat(
        ...
        _frameRate, media::PIXEL_FORMAT_NV12A);
  ...
  if (CVPixelBufferRef pixelBuffer =
    ...
    if (_useGPUMemoryBuffer) {
      if (CVPixelBufferGetIOSurface(pixelBuffer) &&
          videoPixelFormat == media::PIXEL_FORMAT_NV12A) {
