...
void VideoCaptureDeviceApple::GetPhotoState(GetPhotoStateCallback callback) {
  ...
  photo_state->current_blur_mode = blur_mode_;
  photo_state->supported_blur_modes =
        std::vector<media::mojom::BlurMode>(
            {media::mojom::BlurMode::NONE,
             media::mojom::BlurMode::LIGHT,
             media::mojom::BlurMode::STRONG});
  
  photo_state->current_segmentation_mode = segmentation_mode_;
  photo_state->supported_segmentation_modes =
...
void VideoCaptureDeviceApple::SetPhotoOptions(
  ...
  if (settings->has_segmentation_mode && capture_device_) {
    [capture_device_ setSegmentationMode:settings->segmentation_mode];
  }
  if (settings->has_blur_mode && capture_device_) {
    [capture_device_ setBlurMode:settings->blur_mode];
  }
