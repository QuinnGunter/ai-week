...
void SampleBufferTransformer::SetSegmentationMode(media::mojom::SegmentationMode segmentation_mode){
  segmentation_mode_.store(segmentation_mode);
}
...
void SampleBufferTransformer::Reconfigure(
  ...
  staging_pixel_buffer_pool_ = PixelBufferPool::Create(
      kCVPixelFormatType_32BGRA, destination_size_.width(),
      destination_size_.height(), buffer_pool_size);
...
std::span<const std::uint8_t> SampleBufferTransformer::PerformSegmentation(CVPixelBufferRef pixel_buffer, gfx::Size destination_size) {
  DCHECK(pixel_buffer);
  // perform segmentation on converted frame.
  if (CVPixelBufferLockBaseAddress(pixel_buffer, 0) !=
      kCVReturnSuccess) {
    LOG(ERROR)
        << "CVPixelBufferLockBaseAddress for staging_pixel_buffer failed";
    return std::span<const std::uint8_t>();
  }

  uint8_t* base_address = static_cast<uint8_t*>(
      CVPixelBufferGetBaseAddress(pixel_buffer));
  segmentor_.SegmentFrame(
      base_address, destination_size.width(), destination_size.height(),
      media::PIXEL_FORMAT_BGRA, media::mojom::SegmentationMode::SILHOUETTE,
      mmhmm::Segmentor::MaskType::uint8_mask);

  if (CVPixelBufferUnlockBaseAddress(pixel_buffer, 0) !=
      kCVReturnSuccess) {
    LOG(ERROR)
        << "CVPixelBufferUnlockBaseAddress for staging_pixel_buffer failed";
  }
  
  return segmentor_.GetUint8Mask();
}
...
SampleBufferTransformer::Transform(CVPixelBufferRef pixel_buffer) {
  ...
  int size = destination_size_.width() * destination_size_.height();

  auto current_segmentation_mode =
      segmentation_mode_.load(std::memory_order_acquire);
  
  std::span<const std::uint8_t> mask;
  
  if (current_segmentation_mode == media::mojom::SegmentationMode::SILHOUETTE) {
    base::apple::ScopedCFTypeRef<CVPixelBufferRef> staging_pixel_buffer =
        staging_pixel_buffer_pool_->CreateBuffer();

    // Do accelerated color convert
    TransformPixelBufferWithPixelTransfer(destination_pixel_buffer.get(),
                                          staging_pixel_buffer.get());
    
    mask = PerformSegmentation(staging_pixel_buffer.get(), destination_size_);
  }

  if (CVPixelBufferLockBaseAddress(destination_pixel_buffer.get(), 0) !=
      kCVReturnSuccess) {
    LOG(ERROR)
        << "CVPixelBufferLockBaseAddress for destination_pixel_buffer failed";
  }
  DCHECK(CVPixelBufferIsPlanar(destination_pixel_buffer.get()));
  DCHECK_EQ(CVPixelBufferGetPlaneCount(destination_pixel_buffer.get()), 3U);
  constexpr static std::size_t alpha_plane_index = 2;
  DCHECK_EQ(
      CVPixelBufferGetWidthOfPlane(destination_pixel_buffer.get(), alpha_plane_index),
      static_cast<size_t>(destination_size_.width()));
  DCHECK_EQ(CVPixelBufferGetHeightOfPlane(destination_pixel_buffer.get(),
                                          alpha_plane_index),
            static_cast<size_t>(destination_size_.height()));

  uint8_t* alpha = static_cast<uint8_t*>(CVPixelBufferGetBaseAddressOfPlane(
      destination_pixel_buffer.get(), alpha_plane_index));

  if (current_segmentation_mode == media::mojom::SegmentationMode::SILHOUETTE) {
    memcpy(alpha, mask.data(), mask.size());
  } else {
    memset(alpha, 255, size);
  }
  if (CVPixelBufferUnlockBaseAddress(destination_pixel_buffer.get(), 0) !=
      kCVReturnSuccess) {
    LOG(ERROR)
        << "CVPixelBufferUnlockBaseAddress for destination_pixel_buffer failed";
  }
