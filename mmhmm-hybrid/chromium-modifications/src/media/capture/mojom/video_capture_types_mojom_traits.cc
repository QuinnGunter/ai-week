...
EnumTraits<media::mojom::VideoCaptureError, media::VideoCaptureError>::ToMojom(
  ...
  switch (input) {
    ...
    case media::VideoCaptureError::kWinMediaFoundationMmhmmDeviceInitializationFailed:
      return media::mojom::VideoCaptureError::
          kWinMediaFoundationMmhmmDeviceInitializationFailed;
  }
  NOTREACHED();
}
...
bool StructTraits<media::mojom::VideoCaptureParamsDataView,
  ...
  if (!data.ReadPowerLineFrequency(&out->power_line_frequency))
    return false;
  out->enable_segmentation = data.enable_segmentation();
  out->enable_face_detection = data.enable_face_detection();
  out->is_high_dpi_enabled = data.is_high_dpi_enabled();
  // MMHMM: Deserialize stream_configuration_json
  // For nullable string, read into optional then extract value (null -> empty string)
  std::optional<std::string> stream_config;
  if (!data.ReadStreamConfigurationJson(&stream_config))
    return false;
  out->stream_configuration_json = stream_config.value_or("");
  return true;
}
