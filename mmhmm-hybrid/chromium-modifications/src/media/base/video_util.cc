...
scoped_refptr<VideoFrame> ConvertToMemoryMappedFrame(
  ...
  scoped_refptr<VideoFrame> mapped_frame = nullptr;

  if (video_frame->format() == VideoPixelFormat::PIXEL_FORMAT_BGRA ||
      video_frame->format() == VideoPixelFormat::PIXEL_FORMAT_ARGB) {
    ...
    mapped_frame = VideoFrame::WrapExternalYuvDataWithLayout(
        video_frame->layout(), video_frame->visible_rect(),
      video_frame->natural_size(), planes[0], planes[1],
      planes[2], video_frame->timestamp());
  }
