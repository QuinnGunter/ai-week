...
void NativeWidgetMacNSWindowHost::OnVisibilityChanged(bool window_visible) {
  ...
  // Treat miniaturized as visible to keep the compositor alive.
  bool window_visible_or_miniaturized = window_visible || is_miniaturized_;
  is_visible_ = window_visible_or_miniaturized;
  if (compositor_) {
    layer()->SetVisible(window_visible_or_miniaturized);
    if (window_visible_or_miniaturized) {
      compositor_->Unsuspend();
  ...
  widget->OnNativeWidgetVisibilityChanged(window_visible_or_miniaturized);
